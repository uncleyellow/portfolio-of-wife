...
(function () {
  ...
  function handleQuickReply(q) {
    ...
    if (q.id === 'zalo-direct') {
      respondWithTyping(() => {
        const el = addBubble(
          lang === 'en'
            ? 'Great — tap the button below to chat with Thúy directly on Zalo!'
            : 'Tuyệt vời — bấm nút bên dưới để nhắn trực tiếp cho Thúy qua Zalo nhé!',
          'bot'
        );
  ...
  function greetIfNeeded() {
    ...
    setTimeout(() => {
      removeTyping();
      addBubble(escapeHtml(
        lang === 'en'
          ? "Hi! I'm Minh Thúy's virtual assistant 👋 Ask me about her experience, fees, schedule, or certificates — or tap a quick question below."
          : 'Xin chào! Mình là trợ lý ảo của Minh Thúy 👋 Hỏi mình về kinh nghiệm, học phí, lịch dạy, chứng chỉ... hoặc bấm nhanh câu hỏi bên dưới nhé.'
      ), 'bot');
    }, 500);
  }
