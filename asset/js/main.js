...
let charts = {};
if (window.Chart) {
	const rose = getComputedStyle(document.documentElement)
		.getPropertyValue('--rose')
		.trim() || '#D6336C';
	const roseDark = getComputedStyle(document.documentElement)
		.getPropertyValue('--rose-dark')
		.trim() || '#7A1E4B';
	const gold = getComputedStyle(document.documentElement)
		.getPropertyValue('--gold')
		.trim() || '#D9A441';
	const plumMuted = getComputedStyle(document.documentElement)
		.getPropertyValue('--plum-muted')
		.trim() || '#8B5A72';
		...
		charts.students = new Chart(studentsCtx, {
			...
			, data: {
				...
				, datasets: [{
					...
					, borderColor: roseDark
					, backgroundColor: 'rgba(214,51,108,.15)'
					, fill: true
					, tension: 0.4
		...
		charts.radar = new Chart(radarCtx, {
			...
			, data: {
				...
				, datasets: [{
					...
					, data: [88, 87, 92, 85]
					, backgroundColor: 'rgba(214,51,108,.2)'
					, borderColor: rose
					, pointBackgroundColor: rose
          }]
			}
...
const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)')
	.matches;
if (tiltEl && !prefersReducedMotion && window.matchMedia('(pointer:fine)')
	.matches) {
...
window.showToast = function (viText, enText) {
	...
	if (!toast) {
		...
		toast.id = 'mini-toast';
		toast.style.cssText = 'position:fixed;left:50%;bottom:6.5rem;transform:translateX(-50%) translateY(20px);' + 'background:#3B0A28;color:#fff;padding:.65rem 1.1rem;border-radius:999px;font-size:.82rem;' + 'z-index:200;opacity:0;transition:opacity .3s ease, transform .3s ease;box-shadow:0 12px 24px -10px rgba(0,0,0,.4);pointer-events:none;';
...
if (videoPlaceholder) {
	const openVideo = () => {
		window.showToast('Đây là bản demo — hãy nhúng video YouTube thật của bạn (xem ghi chú trong index.html).', 'This is a demo — embed your real YouTube video (see the note in index.html).');
	};
...
if (testiTrack && testiPrev && testiNext) {
	const scrollAmount = () => testiTrack.querySelector('.testi-card')
		?.offsetWidth + 24 || 300;
